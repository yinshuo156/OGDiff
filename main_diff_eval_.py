...
#         save_time(f'set state {time.time() - start}')
...
#                     device=device
#                 )
#                 save_time(f'generate condition vector runtime{time.time() - start2}')
#                 # # --- 【调试代码 2：检查条件向量】 ---
...
#                     )
#                     current_loss += loss_bcls
#                 save_time(f'train diffusion runtime:{time.time() - start3}')
...
#             save_time(f'before eval {time.time() - start}')
...
#                 )
#             save_time(f'eval time{time.time() - start4}')
...
#         save_time(f'epoch runtime:{time.time() - start}\n\n')
